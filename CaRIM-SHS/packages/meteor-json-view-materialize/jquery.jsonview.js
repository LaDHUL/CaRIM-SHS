...
(function(jQuery) {
	...
	JSONFormatter = (function() {
		...
		JSONFormatter.prototype.arrayToHTML = function(array, level) {
			...
			for (index = _i = 0, _len = array.length; _i < _len; index = ++_i) {
				value = array[index];
				hasContents = true;
				output += '<li class=\"list-group-item\">' + this.valueToHTML(value, level + 1);
				/*if (numProps > 1) {
				    output += ',';
				}*/
				output += '</li>';
			...
			if (hasContents) {
				collapsible = level === 0 ? '' : ' can-collapse';
				return "<ul class=\"list-group array level" + level + collapsible + "\">" + output + "</ul>";
			} else {
				return '[ ]';
			}
		};
		JSONFormatter.prototype.objectToHTML = function(object, level) {
			...
			if (hasContents) {
				collapsible = level === 0 ? '' : ' can-collapse';
				return "<ul class=\"list-group obj level" + level + collapsible + "\">" + output + "</ul>";
			} else {
				return '';
			}
		};
	...
	Collapser = (function() {
		...
		Collapser.bindEvent = function(item, options) {
			...
			collapser.className = 'collapser material-icons';
			collapser.innerHTML = options.collapsed ? '<i class="fa fa-plus-square-o" aria-hidden="true"></i>' : '<i class="fa fa-minus-square-o" aria-hidden="true"></i>';
			collapser.addEventListener('click', (function(_this) {
				return function(event) {
					return _this.toggle(event.target.parentNode, options);
				};
			})(this));
			item.insertBefore(collapser, item.firstChild);
			//item.insertBefore(collapser, item.firstChild.nextSibling);
		...
		Collapser.expand = function(collapser) {
			...
			return collapser.innerHTML = '<i class="fa fa-minus-square-o" aria-hidden="true"></i>';
		};
		Collapser.collapse = function(collapser) {
			...
			return collapser.innerHTML = '<i class="fa fa-plus-square-o" aria-hidden="true"></i>';
		};
		...
		Collapser.collapseTarget = function(collapser) {
			var target, targets;
			targets = collapser.parentNode.getElementsByClassName('can-collapse');
			if (!targets.length) {
				return;
			}
	...
	JSONView = {
		collapse: function(el) {
			if (el.innerHTML === '<i class="fa fa-minus-square-o" aria-hidden="true"></i>') {
				return Collapser.collapse(el.nextSibling);
			}
		},
		expand: function(el) {
			if (el.innerHTML === '<i class="fa fa-plus-square-o" aria-hidden="true"></i>') {
				return Collapser.expand(el.nextSibling);
			}
		},
		toggle: function(el) {
	...
	return $.fn.JSONView = function() {
		...
		if (JSONView[args[0]] != null) {
			...
			return this.each(function() {
				...
				if (args[1] != null) {
					level = args[1];
					return $this.find(".jsonview .can-collapse.level" + level)
						.siblings('.collapser')
						.each(function() {
							return JSONView[method](this);
						});
				} else {
					return $this.find('.jsonview > ul > li .can-collapse')
						.siblings('.collapser')
						.each(function() {
							return JSONView[method](this);
						});
			...
			return this.each(function() {
				...
				$this = $(this);
				$this.html(outputDoc);
				items = $this[0].getElementsByClassName('can-collapse');
				_results = [];
